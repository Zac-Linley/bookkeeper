    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-gray-50 dark:bg-gray-900">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">记一笔</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">简单好用的记账工具</p>
        </div>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 mb-5">
          <button onClick={() => switchTab('login')}
            className={`flex-1 text-center pb-3 text-sm font-medium border-b-2 transition-colors ${
              tab === 'login' ? 'text-primary-600 border-primary-600' : 'text-gray-400 dark:text-gray-500 border-transparent'
            }`}>登录</button>
          <button onClick={() => switchTab('register')}
            className={`flex-1 text-center pb-3 text-sm font-medium border-b-2 transition-colors ${
              tab === 'register' ? 'text-primary-600 border-primary-600' : 'text-gray-400 dark:text-gray-500 border-transparent'
            }`}>注册</button>
        </div>
        ...
        {(twoFactorEmail && tab === 'login') ? (
          ...
          <form onSubmit={async (e) => {
            ...
            <button type="button" onClick={() => { setTwoFactorEmail(''); setTwoFactorCode(''); }}
              className="w-full text-xs text-gray-400 dark:text-gray-500">返回登录</button>
          </form>
        ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Email */}
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">邮箱</p>
            <div className="flex items-center gap-2 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-3 bg-white dark:bg-gray-900">
              <IconMail size={16} stroke={1.5} className="text-gray-400 dark:text-gray-500 shrink-0" />
              <input type="email" className="flex-1 outline-none text-sm bg-transparent placeholder-gray-300 dark:placeholder-gray-500"
          ...
          {/* Password */}
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">密码</p>
            <div className="flex items-center gap-2 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-3 bg-white dark:bg-gray-900">
              <IconLock size={16} stroke={1.5} className="text-gray-400 dark:text-gray-500 shrink-0" />
              <input type={showPassword ? 'text' : 'password'}
                className="flex-1 outline-none text-sm bg-transparent placeholder-gray-300 dark:placeholder-gray-500"
          ...
          {tab === 'register' && (
            <>
              <div>
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-3 bg-white dark:bg-gray-900">
                  <input type="text" className="w-full outline-none text-sm bg-transparent placeholder-gray-300 dark:placeholder-gray-500"
                    placeholder="你的昵称" value={displayName}
                    onChange={e => setDisplayName(e.target.value)} />
                </div>
              </div>
              <div>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">确认密码</p>
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-3 bg-white dark:bg-gray-900">
                  <input type={showPassword ? 'text' : 'password'}
                    className="w-full outline-none text-sm bg-transparent placeholder-gray-300 dark:placeholder-gray-500"
